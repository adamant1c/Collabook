...
def display_story_card(story, lang):
...
def show_world_creation_form(lang):
    """World creation form (admin only)"""
    create_world_label = "Create a New World" if lang == Language.EN else "Crea un Nuovo Mondo"
    st.subheader(create_world_label)
    
    with st.form("world_form"):
        title_label = "World Title" if lang == Language.EN else "Titolo del Mondo"
        title = st.text_input(title_label, placeholder="e.g., The Shattered Realms")
        
        genre_label = "Genre" if lang == Language.EN else "Genere"
        ...
        desc_label = "World Description" if lang == Language.EN else "Descrizione del Mondo"
        placeholder = "Describe the setting, rules, atmosphere, and key features of this world..." if lang == Language.EN else "Descrivi l'ambientazione, le regole, l'atmosfera e le caratteristiche principali di questo mondo..."
        world_description = st.text_area(desc_label, placeholder=placeholder, height=200)
        
        button_text = "🌍 Create World" if lang == Language.EN else "🌍 Crea Mondo"
        ...
        if submitted and title and world_description:
            try:
                ...
                success_msg = f"✓ World '{title}' created!" if lang == Language.EN else f"✓ Mondo '{title}' creato!"
                st.success(success_msg)
                st.rerun()
            except Exception as e:
                error_text = "Error" if lang == Language.EN else "Errore"
